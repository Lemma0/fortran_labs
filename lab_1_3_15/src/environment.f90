...
contains
   ...
   pure function Int_plus_string(intt, str) result(res)
      integer, intent(in)                                          :: intt
      character(*), intent(in)                                     :: str
      character(len(str)+Max(Floor(Log10(Real(intt, I_*2)))+1, 1)) :: res

      write (res, '(i0, a)') intt, str
   end function Int_plus_string

   pure function String_plus_int(str, intt) result(res)
      character(*), intent(in)                                     :: str
      integer, intent(in)                                          :: intt
      character(len(str)+Max(Floor(Log10(Real(intt, I_*2)))+1, 1)) :: res

      write (res, '(a, i0)') str, intt
   ...
   subroutine Handle_IO_status(IO, where)
      ...
      select case(IO)
         ...
         case default
            write (ERROR_UNIT, '(a, i0)') "Undetermined behavior has been reached while " // where // ": ", IO
      end select
